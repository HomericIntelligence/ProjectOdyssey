...
fn _softplus_helper(x: ExTensor) raises -> ExTensor:
    """Compute stable softplus: sp(x) = max(0, x) + log(1 + exp(-|x|)).

    This helper extracts the common softplus computation used by both mish()
    and mish_backward() to avoid code duplication while maintaining numerical
    stability through the max(0, x) + log(1 + exp(-|x|)) formulation.

    Args:
            x: Input tensor of any shape.

    Returns:
            Tensor containing stable softplus applied element-wise.
    """
    ...
    return add(x_pos, log_term)  # max(0,x) + log(1 + exp(-|x|))
...
fn mish(tensor: ExTensor) raises -> ExTensor:
    ...
    """
    var softplus = _softplus_helper(tensor)
...
fn mish_backward(
    ...
    Returns:
            Gradient with respect to input.
    """
    var softplus = _softplus_helper(x)
