...
5. **Optimizers** (`shared/training/optimizers/`)
...
2. **Initializers** (`shared/core/initializers.mojo`)
...
2. Implement `conv2d_backward(grad_output, input, kernel, stride, padding) -> (grad_input, grad_kernel, grad_bias)`
3. Implement `maxpool2d_backward(grad_output, input, kernel_size, stride, padding) -> grad_input`
4. Implement `avgpool2d_backward(grad_output, input, kernel_size, stride, padding) -> grad_input`
...
2. Implement `cross_entropy_backward(predictions, targets, reduction='mean') -> grad_predictions`
...
7. **Initializer Tests** (`test_initializers.mojo`)
   - xavier_uniform, xavier_normal
   - kaiming_uniform, kaiming_normal
   - uniform, normal, constant
   - Statistical properties (mean, std)

8. **Comparison Tests** (`test_comparison.mojo`)
   - equal, not_equal
   - less, less_equal, greater, greater_equal
   - Broadcasting behavior

9. **Broadcasting Tests** (`test_broadcasting.mojo`)
   - broadcast_shapes
...
2. Mish: `mish(x) = x * tanh(softplus(x))`
3. ELU: `elu(x, alpha) = x if x > 0 else alpha * (exp(x) - 1)`
4. All backward passes
...
2. **Test First** - Write comprehensive tests (TDD)
3. **Implement** - Write the implementation
4. **Validate** - Run tests, PyTorch validation, gradient checking
5. **Document** - Update documentation and examples
6. **Commit** - Commit with clear message following conventional commits
...
2. **Start Phase 1**: Implement backward passes for linear, conv2d, and pooling
3. Create test files for backward pass validation
4. Run gradient checking against finite differences
5. Validate against PyTorch backward passes
